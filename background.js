...
function searchThroughJSON(keyword) {

    for (let i = 0; i < data.length; i++) {
        if (keyword == data[i].Keyword)
            console.log(data[i].Address);//ovo hiperlinkujes
    }

};

//wrap funkcija za GetTableFromExcel
function search() {
    let x = document.getElementById("polje");
    searchThroughJSON(x.value);
}
//SUIIIIIII